...
const ApplycationsList = () => {
  ...
  const columns = [
    ...
    { field: "chooseRole", headerName: "chooseRole", width: 170 },
  ...
  const handleDelete = async (id) => {
    const defaultOptions = {
      ...
      onSubmit: async () => {
        try {
          await deleteDoc(doc(db, "Applycations", id));
      ...
      renderCell: (params) => {
        return (
          <div className="cellAction">
            <Link to={`/Applycations/${params.row.id}`} style={{ textDecoration: "none" }} state={{ id: params.row.id }}>
  ...
  return (
    <div className='ApplycationsList_comp'>
...
export default ApplycationsList
